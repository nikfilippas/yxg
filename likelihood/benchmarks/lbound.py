...
for s, sur in enumerate(surveys):
    ...
    for i, n in enumerate(npoints):
        # 32 points log sampled in (zmin, zmax)
        # 2048 points linearly sampled in (0.1*zmin, zmax)
...
for i, row in enumerate(axes):
    for j, ax in enumerate(row):
        ...
        if j == 0: ax.set_ylabel("$\\| 1 - \\frac{N_{32}}{N_{2048}} \\|$",
                       fontsize=15)
        ax.hlines(0.01, 0, len(l_arr), linestyle=":")

axes = axes.flatten()
for i, ax in enumerate(axes):
    ax.loglog(l_arr, ratio[i, 0, :], "g-", lw=3, label="%s" % surveys[i])
    ax.legend(loc="lower right")

    ax.xaxis.set_major_formatter(ScalarFormatter())

fig.show()
fig.savefig("lbound_%s.pdf" % profplot, dpi=1000, bbox_inches="tight")