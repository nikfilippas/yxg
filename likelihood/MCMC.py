...
results = ft.MCMC(survey, sprops, cosmo, p0, ft.lnprob, args=(ft.lnprior,),
               nwalkers=nwalkers, nsteps=nsteps)



import numpy as np
samples = results.chain[:, cutoff:, :].reshape((-1, len(p0)))
...
for i in range(results.dim):
    for j in range(results.k):
        ax[i].plot(results.chain[j, :, i], "k-", lw=0.5, alpha=0.2)
