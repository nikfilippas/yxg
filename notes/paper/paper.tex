...
\section{Theory}\label{sec:theory}
  ...
  \subsection{Halo model predictions}\label{ssec:theory.hm}
    ...
    \subsubsection{Galaxies and the halo occupation distribution}\label{sssec:theory.hm.hod}
      ...
      \begin{equation}
        \langle N_s(M)\rangle=\Theta(M-M_0)\,\left(\frac{M-M_0}{M_1'}\right)^{\alpha}.
      \end{equation}
      This is the same model used by \TODO{cite}. For simplicity, we will fix $M_0$ to $M_{\rm min}$, $\sigma_{\rm lnM}=0.15$ and $\alpha$=1 \TODO{cite}, leaving only two free parameters: $M_{\rm min}$ and $M_1$.
    ...
    \subsubsection{tSZ and pressure profiles}\label{sssec:theory.hm.pe}
      ...
      \end{equation}
      where $r_{R00c}$ is the cluster radius enclosing an overdensity of 500 times the critical density (see Section \ref{sssec:theory.hm.cm}). The normalization $P_*$ is given by
      \begin{equation}
      ...
      \TODO{describe variance and covariance with g}
              
      
    \subsubsection{Concentration-mass relation}\label{sssec:theory.hm.cm}
      \TODO{describe how we homogenize $c(M)$.}
