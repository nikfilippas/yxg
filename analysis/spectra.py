...
class Spectrum(object):
    ...
    @classmethod
    def from_fields(Spectrum,field1,field2,bpws,wsp=None,save_windows=True):
        ...
        cell=wsp.decouple_cell(nmt.compute_coupled_cell(field1.field,field2.field))[0]

        #Compute noise power spectrum if needed
        if field1.is_ndens and field2.is_ndens and field1.name==field2.name:
            nell=wsp.decouple_cell(wsp.couple_cell([np.ones(3*field1.nside)/field1.ndens]))[0]
        else:
    ...
    def from_file(Spectrum,fname,name1,name2):
        d=np.load(fname)
        return Spectrum(name1,name2,d['ls'],d['nls'],d['cls'],d['windows'])
