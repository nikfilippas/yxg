...
\section{Methods}\label{sec:methods}
  ...
  \subsection{Systematics treatment}\label{ssec:methods.syst}
    ...
    \subsubsection{Redshift distribution uncertainties}\label{sssec:methods.syst.photoz}
      ...
      In particular, our model of the $y\times \delta_g$ cross-correlation is very sensitive to the width of the redshift distributions. This is because wider redshift distributions, which imply lower clustering amplitude, affect the amplitude of the galaxy auto-correlation, $\delta_g\times \delta_g$. At the same time, the $y$ auto-spectrum is almost insensitive to clustering; therefore the width only affects the $y\ times \delta_g$ cross-correlation in our analysis. We introduce an additional parameter, $w_z$, that stretches the support of the redshift distribution while preserving unit total probability. Concretely, given a fiducial distribution $p_{\rm fid}(z)$ with mean redshift $\bar{z}$, $w_z$ is implemented as
      \begin{equation}
